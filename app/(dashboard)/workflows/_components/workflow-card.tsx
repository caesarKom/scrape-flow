...
import {
  ExecutionStatusIndictor,
  ExecutionStatusLabel,
} from "@/app/workflow/runs/[workflowId]/_components/execution-status-indictor"
...
export const WorkflowCard = ({ workflow }: { workflow: Workflow }) => {
  ...
  return (
    <Card className="border border-separate shadow-sm rounded-lg overflow-hidden hover:shadow-md dark:shadow-primary/30 group/card">
      ...
      <LastRunDetalis workflow={workflow} />
    </Card>
  )
}
...
function LastRunDetalis({ workflow }: { workflow: Workflow }) {
  const isDraft = workflow.status === WorkflowStatus.DRAFT
  if (isDraft) {
    return null
  }
  ...
  const nextScheduleUTC =
    nextRunAt && formatInTimeZone(nextRunAt, "Europe/Berlin", "HH:mm")
    ...
    <div className="bg-primary/5 px-4 py-1 flex justify-between items-center text-muted-foreground">
      ...
      {nextRunAt && (
        <div className="flex items-center text-sm gap-2">
          ...
          <span className="text-xs">{nextScheduleUTC}</span>
        </div>
      )}
    </div>
  )
